...
void test_intersection()
{
    ...
    struct Point B2 = {2, 2};
    assert(line_intersect(A, B, A2, B2) == false);

    // simple intersection
    struct Point A3 = {-5, 5};
    struct Point B3 = {5, -5};
    assert(line_intersect(A, B, A3, B3) == true);

    // endpoint intersection
    struct Point A4 = {-7, 7};
    assert(line_intersect(A, B, A4, A) == true);

    struct Point B4 = {7, -7};
    assert(line_intersect(A, B, B, B4) == true);

    // parallel lines
    struct Point A5 = {-4, -5};
    struct Point B5 = {6, 5};
    assert(line_intersect(A, B, A5, B5) == false);

    // non-intersecting
    struct Point A6 = {9, 0};
    struct Point B6 = {14, 1};
    assert(line_intersect(A,B, A6, B6) == false);

    // long lines
    struct Point A7 = {-1.2E+8, -3.1E+2};
    struct Point B7 = {2.2E+122, 1.8E+31};
    assert(line_intersect(A,B, A7, B7) == false);
